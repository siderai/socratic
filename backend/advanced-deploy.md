...
## Продвинутая инструкция по развертыванию bot-example
...
```sql
CREATE USER bot_example_user PASSWORD '<GENERATE>';
CREATE DATABASE bot_example_db;
GRANT ALL PRIVILEGES ON DATABASE bot_example_db 
  TO bot_example_user;
```
...
```bash
git clone <THIS_REPOSITORY> /opt/express/bots/bot-example
cd /opt/express/bots/bot-example
```
...
4. В директории `/opt/express/bots/bot-example` создайте файл
